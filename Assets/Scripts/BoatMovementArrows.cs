...
public class BoatMovementArrows : MonoBehaviour
{
    ...
    // Hàm để AI mô phỏng điều khiển như người chơi
public void SimulateInput(bool forward, bool backward, bool left, bool right)
{
    motorInput = 0f;
    if (forward) motorInput = 1f;
    else if (backward) motorInput = -1f;

    steerInput = 0f;
    if (left) steerInput = -1f;
    else if (right) steerInput = 1f;
}
} 