...
public class BoatMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (rb == null)
        {
            Debug.LogError("BoatMovement cần có Rigidbody component!");
            return;
        }
    ...
    void ApplyMotor()
    {
        ...
        {
            // Sử dụng hướng của thuyền thay vì Vector3.forward cố định
    ...
    void ApplySteering()
    {
        if (Mathf.Abs(steerInput) > 0.1f && Mathf.Abs(motorInput) > 0.1f)
        {
            ...
            Vector3 steerTorque = transform.up * steerAmount; // Có thể cần đổi thành -transform.up nếu steering bị ngược
